  ...
  return (
    <div className="paper-panel overflow-hidden rounded-[2px] border-2 border-dashed">
      <div className="grid min-h-[620px] min-[1080px]:grid-cols-[18rem_minmax(0,1fr)]">
        <aside className="paper-subtle border-r border-dashed border-border p-6">
          <CheckCircle2 aria-hidden="true" className="size-6 text-accent" />
          <p className="mt-4 text-xs font-semibold text-accent">本场已完成</p>
          <p className="marker-highlight mt-2 text-4xl font-extrabold tabular-nums text-card-foreground">{average}</p>
          <p className="mt-1 text-xs text-muted">平均得分 / 100</p>
          <dl className="mt-7 grid gap-4 border-y border-dashed border-border py-5 text-xs">
        ...
        <section className="p-7" aria-labelledby="interview-summary-title">
          <h2 id="interview-summary-title" className="doodle-heading text-xl font-extrabold text-card-foreground">本场表现轨迹</h2>
          <p className="mt-2 text-sm text-muted">每道题保留最后一次作答结果，重答版本仍可从历史场次中查看。</p>
          <div className="mt-7 border-y border-dashed border-border">
            {detail.questions.map((question) => {
              const attempt = question.attempts.at(-1);
              if (!attempt) return null;
              return (
                <div key={question.id} className="grid gap-3 border-b border-dashed border-border py-5 last:border-b-0 min-[900px]:grid-cols-[4rem_minmax(0,1fr)_8rem]">
                  <div>
                    <p className="text-xs text-muted">第 {question.roundIndex} 题</p>
                    <p className="mt-1 text-lg font-extrabold tabular-nums text-card-foreground">{interviewOverallScore(attempt.scores)}</p>
                  </div>
          ...
          {gaps.length > 0 ? (
            <div className="mt-7">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {gaps.map((question) => (
                  question.termId ? (
                    <Link key={question.id} href={`/?concept=${question.termId}`} className="doodle-link doodle-row inline-flex h-9 items-center gap-2 rounded-[2px] border border-dashed border-border bg-card px-3 text-xs font-semibold text-foreground hover:bg-highlight/15">
