...
export function AnalyticsView({ initialData }: { initialData: LearningAnalytics }) {
  ...
  return (
    <PageShell
      title="成长分析"
      description="用真实学习证据判断下一步，不用累计数字制造进步感"
      width="xl"
      actions={(
        <SegmentedControl
          value={String(range)}
        ...
        <section className="mt-8" aria-labelledby="trend-heading">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 id="trend-heading" className="mt-1 text-base font-semibold text-foreground">
                每一天做了什么
              </h2>
            </div>
            <TrendLegend />
          ...
          <div className="mt-4 grid min-[1180px]:grid-cols-[minmax(0,1fr)_20rem]">
            <div className="min-w-0 border-y border-border py-5 min-[1180px]:pr-7">
              <ActivityTrend
                days={data.trend}
...
function TodayBand({ data }: { data: LearningAnalytics }) {
  ...
  return (
    <section className="grid overflow-hidden rounded-md border border-border bg-card shadow-sm min-[900px]:grid-cols-[minmax(0,1.2fr)_minmax(31rem,0.8fr)]">
      ...
      <div className="grid grid-cols-2 bg-card text-card-foreground">
...
function LoadValue({
  ...
  return (
    <div className="min-h-28 border-b border-l border-border px-5 py-4 even:border-r-0 [&:nth-child(n+3)]:border-b-0">
...
function TrendLegend() {
  return (
    <div className="flex flex-wrap justify-end gap-x-4 gap-y-1 text-[11px] text-muted" aria-label="趋势图图例">
      ...
      <span className="inline-flex items-center gap-1.5"><i className="size-2 rounded-full bg-danger" />提示次数</span>
    </div>
  );
}
      ...
      <div className="absolute inset-0 flex items-end gap-1.5 pb-7" aria-label="每日学习行为">
        {days.map((day, index) => {
          ...
          return (
            ...
            >
              {day.hints > 0 ? (
                <span className="absolute top-1 flex size-4 items-center justify-center rounded-full bg-danger text-[9px] font-semibold text-danger-foreground">{day.hints}</span>
              ) : null}
...
function WeakSkills({ skills }: { skills: LearningAnalytics['weakSkills'] }) {
  return (
    <aside className="border-b border-border py-5 min-[1180px]:border-l min-[1180px]:border-t min-[1180px]:pl-6">
          ...
          {skills.map((skill) => (
            <article key={skill.conceptId} className="py-3 first:pt-0">
              <div className="flex items-center justify-between gap-3">
                <Link href={skill.href} className="truncate text-sm font-semibold text-foreground hover:text-primary hover:underline">
                  {skill.name}
                </Link>
                <span className="shrink-0 text-[10px] text-muted">{skill.sampleSize} 条行为证据</span>
              </div>
              <div className="mt-2 h-1 overflow-hidden rounded-full bg-surface">
                <div className="h-full rounded-full bg-danger" style={{ width: `${Math.min(100, Math.max(12, skill.priority))}%` }} />
              </div>
              <p className="mt-2 line-clamp-2 text-[11px] leading-4 text-muted">{skill.evidence.join('；')}</p>
              <Link href={skill.actionHref} className="mt-2 inline-flex items-center gap-1 text-[11px] font-semibold text-primary hover:underline">
...
function MetricStrip({ metrics }: { metrics: LearningAnalytics['metrics'] }) {
  return (
    <section className="mt-8 grid border-y border-border min-[780px]:grid-cols-2 min-[1180px]:grid-cols-4" aria-label="关键学习指标">
      {metrics.map((metric, index) => (
        <Link
          key={metric.id}
          href={metric.href}
          className={`group min-h-36 px-5 py-4 transition-colors hover:bg-card ${index > 0 ? 'border-t border-border min-[780px]:border-l min-[780px]:border-t-0' : ''} ${index === 2 ? 'min-[780px]:border-l-0 min-[1180px]:border-l' : ''}`}
        >
          <div className="flex items-center justify-between gap-3">
...
function ProgressStrip({ progress }: { progress: LearningAnalytics['progress'] }) {
  ...
  return (
    <section className="mt-8" aria-labelledby="progress-heading">
      ...
      <div className="mt-3 grid divide-y divide-border border-y border-border min-[900px]:grid-cols-3 min-[900px]:divide-x min-[900px]:divide-y-0">
        {progress.map((item) => (
  ...
  return (
    <section id="activity-ledger" className="mt-8 scroll-mt-20" aria-labelledby="activity-heading">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 id="activity-heading" className="text-base font-semibold text-foreground">证据流水</h2>
      ...
      {activities.length === 0 ? (
        <div className="mt-3 border-y border-border py-12 text-center">
          <MessageSquareText aria-hidden="true" className="mx-auto size-5 text-muted" />
        ...
        <div className="mt-3 overflow-hidden border-y border-border">
          <div className="grid grid-cols-[9rem_minmax(12rem,1fr)_minmax(10rem,0.9fr)_8rem] gap-4 border-b border-border bg-surface px-3 py-2 text-[10px] font-semibold text-muted">
            ...
            {activities.map((activity) => {
              ...
              return (
                <Link
                  key={activity.id}
                  href={activity.href}
                  className="group grid min-h-12 grid-cols-[9rem_minmax(12rem,1fr)_minmax(10rem,0.9fr)_8rem] items-center gap-4 px-3 py-2 text-xs transition-colors hover:bg-card"
                >
