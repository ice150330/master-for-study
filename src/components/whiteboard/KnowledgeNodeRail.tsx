...
export function KnowledgeNodeRail({
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col border-l border-border bg-card" aria-label="知识节点详情">
      <header className="flex items-start justify-between gap-3 border-b border-border px-5 py-4">
        <div className="min-w-0">
          <p className="text-[11px] font-medium text-muted">{node.kind === 'domain' ? '知识领域' : node.kind === 'session' ? '会话节点' : '知识对象'}</p>
          <h2 className="mt-1 truncate text-base font-semibold text-card-foreground">{node.label}</h2>
        </div>
        <IconButton label="关闭节点详情" onClick={onClose}><X aria-hidden="true" /></IconButton>
      </header>
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        {node.masteryState ? <span className="rounded-md bg-primary/10 px-2 py-1 text-[11px] font-medium text-primary">{masteryLabel[node.masteryState]}</span> : null}
            ...
            <div className="mt-3 grid grid-cols-3 gap-x-3 gap-y-4">
              {evidence.map(([label, value, Icon]) => (
                <div key={label}>
                  ...
                  <strong className="mt-1 block font-mono text-base tabular-nums text-card-foreground">{value}</strong>
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="mt-5 border-t border-border pt-4">
          ...
          {neighbors.length ? <div className="mt-2 space-y-1">
            {neighbors.map(({ edge, node: related }) => (
              <button key={edge.id} type="button" onClick={() => onRecenter(related.id)} className="flex w-full items-center justify-between gap-3 rounded-md px-2 py-2 text-left hover:bg-surface">
      ...
      <div className="shrink-0 border-t border-border p-4">
        ...
        {node.href ? (
          <a href={node.href} className="mt-2 flex h-9 w-full items-center justify-center gap-2 rounded-md bg-primary px-3 text-sm font-medium text-primary-foreground hover:opacity-90">
        ...
        {node.termId ? <div className="mt-2 grid grid-cols-3 gap-1">
          ...
          <RailLink href={`/practice?concept=${node.termId}`} label="练习" />
...
function RailLink({ href, label }: { href: string; label: string }) {
  return <a href={href} className="flex h-8 items-center justify-center rounded-md bg-surface text-[11px] font-medium text-card-foreground hover:bg-card-soft">{label}</a>;
}
