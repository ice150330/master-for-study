...
> 对应 `docs/plans/2026-08-13-项目初期实施计划-design.md` 的阶段 0 + 阶段 1；八大模块中的白板 / 面试 / 隐性巩固 / 成长分析 / 资源库 / 实践区**不在本次范围**（后六模块已于 2026-08-14 随后完成，见 `CHANGES.md`）。

> **状态（2026-08-14）**：阶段 0 + 阶段 1 已完成；除 P1/P4 与「验收自查」中的流式对话复验外，任务均已勾选完成。
...
- [x] **测试**：引入 Vitest 覆盖 `lib/` 纯函数——会话树组装、术语标注解析、FSRS 调度、树布局（`npm test`，4 文件 18 用例通过）。
- [ ] **验收自查**：会话树 / 持久化已实测通过；流式对话与术语高亮曾遇 401，根因已定位为 shell 环境变量 `DEEPSEEK_API_KEY` 覆盖了 `.env` 有效 key（非 key 本身失效），清理后待复验。
- [x] **追加 `CHANGES.md`**：按「倒序记录」约定记录本轮变更。
